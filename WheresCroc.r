...
#' Authors for the code written below:
#' Anna Hillver
#' Linnea Eriksson


makeMoves <- function (listwMoves, currentCrocReadings, posPeople, edges, waterReadings) {
  nrow = 40
  ncol = 40
  if (listwMoves$mem$status == 0) {
    transMatrix = matrix(0, nrow, ncol)
    listwMoves$mem$transMatrix = createTransMatrix(edges, transMatrix)
    listwMoves$mem$probMatrix = getProbMatrix(edges)    
  }
  if (listwMoves$mem$status <= 1) {
    listwMoves$mem$fVals = rep(1/40, 40)
    listwMoves$mem$status = 2
  }
  
  
  obsMatrix = getObservation(posPeople,currentCrocReadings,waterReadings)
  nextfVals = listwMoves$mem$fVals %*% listwMoves$mem$transMatrix %*% obsMatrix
  holeNr1 = which(nextfVals == max(nextfVals))[[1]]
  moveNr1 = listwMoves$mem$probMatrix[posPeople[[3]], holeNr1]
  
  if (moveNr1 == 0) {
    nextfVals[[posPeople[[3]]]] = 0
    holeNr2 = which(nextfVals[-holeNr1] == max(nextfVals[-holeNr1]))[[1]]
    moveNr2 = listwMoves$mem$probMatrix[holeNr1, holeNr2]
  } else {
    moveNr2 = listwMoves$mem$probMatrix[moveNr1, holeNr1]
    if (moveNr2 == 0) {
      nextfVals[[moveNr1]] = 0
    }
  }
  
  nextfVals = nextfVals/sum(nextfVals)
  listwMoves$mem$fVals = nextfVals
  listwMoves$moves = c(moveNr1,moveNr2)
  
  return (listwMoves)
}


createNode <- function (pos, cost, path) {
  return (list(pos = pos, cost = cost, path = path))
}


aStar <- function(edges, probMatrix, from, to) {
  if (from == to) {
    probMatrix[from, to] = 0
    return (probMatrix)
  }
  frontier = list()
  frontier[[1]] = createNode(from, 0, c())
  while (length(frontier) > 0) {
    expanded = frontier[[1]]
    frontier = frontier[-1]
    if (expanded$pos == to) {
      probMatrix[from,to] = expanded$path[[1]]
      return (probMatrix)
    } else {
      frontier = addNeighbourNodesFrontier(edges, frontier, expanded)
    }
  }
}

getNeighbourNodes <- function (edges, node) {
  neighbours = c()
  for (edge in 1:dim(edges)[[1]]) {
    if (edges[edge,1] == node) {
      neighbours = append(neighbours, edges[edge,2])
    } else if (edges[edge,2] == node) {
      neighbours = append(neighbours, edges[edge,1])
    }
  }
  return (neighbours)        
}

addNeighbourNodesFrontier <- function(edges, frontier, from) {
  neighbours = getNeighbourNodes(edges, from$pos)
  for (node in neighbours) {
    frontier = addExpanded(frontier, createNode(node, from$cost + 1, c(from$path, node)))
  }
  return (frontier)
}

addExpanded <- function (frontier, addNode) {
  step = 1
  if (any(addNode$pos == addNode$path[-length(addNode$path)])) {
    return (frontier)
  }
  for (expanded in frontier) {
    if (expanded$pos == addNode$pos) {
      if (addNode$cost < expanded$cost) {
  ...
  for (expanded in frontier) {
    if (addNode$cost < expanded$cost) {
      return (append(frontier, list(addNode), step - 1))
    }
    step = step + 1
  }
  return (append(frontier, list(addNode)))
}



getObservation <- function(posPeople,crocReadings,waterReadings) {
  nrow=40
  ncol=40
  obsMatrix = matrix(0,nrow,ncol)
  importantPeople = posPeople[-3]
  importantPeople = importantPeople[which(!is.na(importantPeople))]
  if (any(importantPeople < 0)) {
    i = importantPeople[which(importantPeople < 0)]
    obsMatrix[i,i] = 1
  } else {
    for (i in 1:nrow) {
      if (!any(importantPeople == i)) {
        probability = 1
        for (reading in 1:3) {
          crocReading = crocReadings[[reading]]
          readingMeanVal = waterReadings[[reading]][i, 1]
          readingStdDevVal= waterReadings[[reading]][i, 2]
          probability = probability * dnorm(crocReading, readingMeanVal, readingStdDevVal)
        }
        obsMatrix[[i,i]] = probability
      }
    }
  }
  return (obsMatrix) 
...
getProbMatrix <- function (edges) {
  nrow = 40
  ncol = 40
  probMatrix = matrix(0,nrow,ncol)
  for (from in 1:nrow) {
    for (to in 1:ncol) {
      probMatrix = aStar(edges, probMatrix, from, to)
    }
  }
  return (probMatrix)
}

createTransMatrix <- function(edges, transMatrix) {
  for (node in 1:40) {
    neighbouringNodePos = c(node, getNeighbourNodes(edges, node))
    for (neighbourPos in neighbouringNodePos) {
      transMatrix[node, neighbourPos] = 1/length(neighbouringNodePos)
    }
  }
  return (transMatrix)
}
...
runWheresCroc(makeMoves, doPlot = T, showCroc = F, pause = 1,verbose = T, returnMem = F, mem = NA)
testWC(makeMoves,verbose=0,returnVec=FALSE,n=500,seed=21,timeLimit=300)
